...
ggplot(aux_BI,aes(x=factor(Amplitud), y=diff))+
  geom_boxplot()+
  geom_hline(yintercept = 0)+
  xlab("Amplitud (Cd/m^2)")+
  ylab("Diff (Cd/m^2)")+
  ggtitle("Boxplot Brigtness Induccion ")
ggsave("figure_3.png")
...
ggplot(aux_BI,aes(x=factor(Amplitud), y=diff))+
  ...
  ggtitle("Boxplot Brigtness Induccion by subject")
ggsave("figure_4.png")
...
geom_boxplot()+
  ...
  facet_wrap(vars(Subject))
ggsave("Figura_10.png")

#lo que podemos ver en estos plots es lo siguiente, 
#es mas comun que haya mas dispersion en cada sujeto cuando mayor es la amplitud
#hay sujetos como BM la dispersion en cuanto aumeta la amplitud en 10 es extrema, 
#sujetos como XO en la que no se puede aprecias que haya suopresion 
#o sujetos como AM en la que no hay diferencia en cuanto pasas de la amplitud 10 
